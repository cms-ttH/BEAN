...
namespace kinem{
  ...
  //  calculate ECAL eta from eta, Z_vertex, and R_plane
  inline double etaEcal(double etaP, double Zvtx, double Rvtx);
  inline double etaEcal2(double etaP, double Zecal, double Recal);
  ...
  inline double correlation(std::vector<double> x_var, std::vector<double> y_var);
...
inline
double kinem::etaEcal(double etaP, double Zvtx, double Rvtx)
{
  const double R_ECAL = 136.5;
  const double Z_Endcap = 328.0;
  const double etaBarrelEndcap = 1.496;

  if( etaP != 0. ){
    double Theta = 0.0;
    double ZEcal = (R_ECAL-Rvtx)*sinh(etaP)+Zvtx;
    ...
    if( fabs(ETA) > etaBarrelEndcap ){
      ...
      if( etaP < 0.0 ) Zend = -Zend;
      double Zlen = Zend - Zvtx;
      double RR = Zlen/sinh(etaP);
      Theta = atan((RR+Rvtx)/Zend);
      if( Theta < 0.0 ) Theta = Theta + PI;
    ...
    std::cout << "[EgammaObjects::ecalEta] Warning: Eta equals to zero, not correcting" << std::endl;
    return etaP;
  }
}
...
inline
double kinem::etaEcal2(double etaP, double Zecal, double Recal)
{
  const double R_ECAL = Recal;
  const double Z_Endcap = 328.0;
  const double etaBarrelEndcap = 1.496;
  ...
  if( etaP != 0. ){
    ...
    if( fabs(ETA) > etaBarrelEndcap ){
      ...
      double Zlen = Zend;// - Zvtx;
      double RR = Zlen/sinh(etaP);
      Theta = atan((RR)/Zend);//atan((RR+Rvtx)/Zend)
      if( Theta < 0.0 ) Theta = Theta + PI;
      ETA = -log(tan(0.5*Theta));
    }
...
inline
double kinem::correlation(std::vector<double> x_var, std::vector<double> y_var)
{

  int N_counter;

  if( x_var.size()==y_var.size() ) N_counter= x_var.size();
  else{
    std::cout <<  " ERROR!! vectors x and y of different sizes" << std::endl;
    N_counter=-1;
    return -999;
  }
  ...
  double mean_x, mean_y, sweep, delta_x, delta_y;
  ...
  for( int j = 1; j<N_counter; j++ ) {

    sweep = double(j)/double(j+1.0);
    delta_x = x_var[j]-mean_x;
    delta_y = y_var[j]-mean_y;
    sum_sq_x += delta_x*delta_x*sweep;
    sum_sq_y += delta_y*delta_y*sweep;
    sum_coproduct += delta_x*delta_y*sweep;
    mean_x += delta_x/double(j+1.0);
    mean_y += delta_y/double(j+1.0);

  }

  double pop_sd_x = sqrt( sum_sq_x/N_counter );
  double pop_sd_y = sqrt( sum_sq_y/N_counter );
  double cov_x_y  = sum_coproduct/N_counter;
