...
BEANhelper::GetCleanJets(const BNjetCollection& jets, const BNleptonCollection& leptons, const float maxDeltaR, std::vector<unsigned int>* jet_indices) {
  ...
  for (auto& lepton: leptons) {
    ...
    for (unsigned i=0; i<cleanJets.size(); i++){
      dR = reco::deltaR(lepton->eta, lepton->phi, cleanJets.at(i).eta, cleanJets.at(i).phi);
      if (dR < minDeltaR) {
        minDeltaR = dR;
        matchIndex = i;
      }
...
bool BEANhelper::IsTauEvent(const BNtauCollection& iTaus, const BNjetCollection& iJets, const BNelectronCollection& iElectrons, const BNmuonCollection& iMuons, const sysType::sysType iSysType){
	return ( IsTauTauLeptonEvent(iTaus, iJets, iElectrons, iMuons, iSysType) || IsTauLeptonLeptonEvent(iTaus, iJets, iElectrons, iMuons, iSysType) );
}

bool
BEANhelper::IsTauLeptonLeptonEvent(const BNtauCollection& iTaus, const BNjetCollection& iJets, const BNelectronCollection& iElectrons, const BNmuonCollection& iMuons, const sysType::sysType iSysType)
{
    ...
    if (!(numTightLeptons > 0) || numLooseLeptons != 2)
        return false;
...
bool
BEANhelper::IsTauTauLeptonEvent(const BNtauCollection& iTaus, const BNjetCollection& iJets, const BNelectronCollection& iElectrons, const BNmuonCollection& iMuons, const sysType::sysType iSysType)
{
	// B-jets: veto out events with 3 or more CSVM-tags
	BNjetCollection correctedJets				= GetCorrectedJets(iJets, iSysType);
	BNjetCollection selectedCorrectedCSVMJets	= GetSelectedJets(correctedJets, 30, 2.4, jetID::jetLoose, 'M');
    ...
    if ((numTightLeptons < 1 || 2 < numTightLeptons) ||
          (numLooseLeptons < 1 || 2 < numLooseLeptons))
        return false;
...
// 	(daughter0ID==91 || daughter0ID==92 || daughter0ID==93 || daughter1ID==91 || daughter1ID==92 || daughter1ID==93) ) list_b_from_t.push_back(i);
  ...
  // Loop over those c's that ARE from b or W
  std::vector<int> list_matched_jets_from_bW;
  std::vector<int> list_c_match_from_bW;
  for( int i=0; i<int(list_c_from_bW.size()); i++ ){
    int ind = list_c_from_bW[i];
    ...
    list_matched_jets_from_bW.push_back(matchedJet);
    list_c_match_from_bW.push_back(ind);
  ...
  for( int i=0; i<int(list_c.size()); i++ ){
    ...
    for( int iJet = 0; iJet < int(list_matched_jets_from_bW.size()); iJet++ ){
      if( matchedJet==list_matched_jets_from_bW[iJet] ){
	previousMatch_from_bW = true;
	break;
      }
    }
    if( previousMatch_from_bW ) continue;
